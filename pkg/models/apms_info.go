...
// ApmsInfo A container for information about a set of APM Servers.
// swagger:model ApmsInfo
...
func (m *ApmsInfo) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateApms(formats); err != nil {
		res = append(res, err)
	}
...
func (m *ApmsInfo) validateApms(formats strfmt.Registry) error {

	if err := validate.Required("apms", "body", m.Apms); err != nil {
		return err
	}

	for i := 0; i < len(m.Apms); i++ {
		if swag.IsZero(m.Apms[i]) { // not required
			continue
		}

		if m.Apms[i] != nil {
			if err := m.Apms[i].Validate(formats); err != nil {
...
func (m *ApmsInfo) validateReturnCount(formats strfmt.Registry) error {
