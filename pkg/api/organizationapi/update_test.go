	...
	requestBody := `
{
   ...
   "operational_contacts" : [
	...
	requestBodyCompact := &bytes.Buffer{}
	json.Compact(requestBodyCompact, []byte(requestBody))
	tests := []struct {
		name   string
		params UpdateParams
				...
				API: api.NewMock(
					mock.New200ResponseAssertion(
						&mock.RequestAssertion{
							...
							Body:   mock.NewStringBody(requestBodyCompact.String() + "\n"),
						},
						mock.NewStringBody(responseBody)),
				),
				OrganizationID:                "testorg",
				Name:                          "testorganization",
				DefaultDiskUsageAlertsEnabled: ec.Bool(true),
				BillingContacts:               []string{"contact-a", "contact-b"},
				AllowedEmailDomains:           []string{"mail@test"},
