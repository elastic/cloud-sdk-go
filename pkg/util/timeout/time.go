...
// DurationFromSize computes a time.Duration from the size, if the computed
// value is smaller than the minimum timeout that is returned instead
func DurationFromSize(size int64) time.Duration {
	const divider = 100
	const multiplier = 2
	var minTimeout = DefaultClientTimeout * time.Second
	var timeout = time.Duration(size/divider*multiplier) * time.Second
